...
@injectable()
export class WaitingForLocationUpdateState extends AbstractState {
  public constructor(
  @inject(SERVICE_IDENTIFIER.ILogger)
    ...
    IErrorWithoutAdditionalHandling,
    @inject(SERVICE_IDENTIFIER.ReadyToUpdateLocationState)
    private readonly readyToUpdateLocationState: IState
  ) {
    super(
      logger,
      errorWithoutAdditionalHandling
  ...
  public async handleInput(
    ...
    if ('location' in message && message.location) {
      ...
      await this.context
        .transitionTo(this.readyToUpdateLocationState, message)
