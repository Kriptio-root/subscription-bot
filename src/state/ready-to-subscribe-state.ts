...
@injectable()
export class ReadyToSubscribeState extends AbstractState {
  public constructor(
  ...
  public async handleInput(
    ...
    await this.context.saveSubscription(message, userData)
    await this.userListCache.addUser(message)
    await this.context.sendMessage(
      message,
      userReplies.successfulSubscription
    )
    await this.context.completeSubscription(message)
  }
}
